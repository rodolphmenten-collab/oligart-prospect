...
function filtered(){const q=($('#search')?.value||'').toLowerCase(), pr=$('#priorityFilter')?.value||'', st=$('#statusFilter')?.value||'', cat=$('#categoryFilter')?.value||'', noAg=$('#noAgencyFilter')?.checked||false;return prospects.filter(p=>(!q||`${p.company} ${p.sector}`.toLowerCase().includes(q))&&(!pr||p.priority===pr)&&(!st||p.status===st)&&(!cat||p.category===cat)&&(!noAg||p.hasAgency===false))}
...
['search','priorityFilter','statusFilter','categoryFilter','noAgencyFilter'].forEach(id=>$('#'+id).addEventListener(id==='search'?'input':'change',renderTableAndBind));
